...
### v0.7.9
- **TOP 主机 8 → 10**，图例同步更名。
- **配色全面回归宿主主题**：撤销全部自定硬编码色（列表/弹窗/JSON 树），统一改用宿主主题变量（背景、边框、文字、状态、品牌色），插件视觉与宿主主题完全一致，不再自配颜色。

### v0.7.8
- **列表交互重设计**：一次点击条目 = 行内轻量展开（主机/通道/耗时/错误摘要，不打断浏览视线，再点收起）；点展开区里的「详情」按钮 = 弹窗——两级信息层级，弹窗只在真正要看完整内容时出现。
- **列表配色重做**：每行左缘状态色条（成功绿/错误红/进行中灰，悬停加宽）、方法徽章化、URL 主角化提亮、状态/时间/流量弱化右对齐；选中行高亮。
- **弹窗样式精修**：顶缘蓝青光带、等宽字体标题、徽章底色化、请求/响应分区头部带色条与余晖分隔线、滚动条主题化、开窗动画升级。

### v0.7.7
- **弹窗稳定性**：详情弹窗持有打开时的条目快照，条目被轮询淘汰时回落显示快照（弹窗不再闪没），仅用户主动关闭消失。
- **配色再调优**：JSON 树换更友好的等宽字体栈（Cascadia/Consolas 优先）、字号 12px、行高 1.7；键 `#59c2ff` / 字符串 `#8ddb8c` / 数字 `#ffb454` / 布尔紫，深底 `#12161f` 上对比度更高；列表 URL/时间文字亮度提升。

### v0.7.6
- **日志详情弹窗**：点击条目改为弹出居中详情卡片（遮罩模糊 + 入场动画，Esc/点遮罩关闭）——头部为完整 URL（截断可悬停），状态/通道/耗时/流量徽标行，请求/响应分节，详情区独立滚动，弹窗打开时列表轮询继续、内容自动刷新。
- **配色可读性**：日志状态色（成功绿/错误红）加亮加粗；JSON 树换高对比配色（键亮蓝加粗/字符串亮绿/数字琥珀/布尔紫），底色加深一档拉开对比；列表条目悬停高亮。

### v0.7.5
- **请求日志单行化**：列表条目长 URL 不再逐字符换行撑高（单行省略号截断），时间/状态/字节恒定居右。
- **JSON 树重做**：行式布局——折叠钮与键左对齐、缩进网格化、逗号挂行尾、行悬停高亮；长字符串（URL/长文本）单行省略号，点击展开全文；非 JSON 文本预览同样单行化与树视图统一观感。

### v0.7.4
- **图表错位修复**：目标主机 TOP 8 的长主机名（如 CDN 域名+端口）逐字符换行，把横条行撑成多行错位；改为单行截断（超出省略号，悬停 title 显示全名），行高恒定不错位。

### v0.7.3
- **严重修复：中继死亡无自愈，宿主全部 fetch 静默失败**——harness 策略经本地中继安装后，若中继意外停止（配置与系统代理均无变化时 `sync` 幂等短路不会重跑），env 永远指向死端口：插件目录等一切走代理的请求瞬间失败（`fetch failed, tried through http://127.0.0.1:<死端口>`）。新增 5 秒存活看护：曾经中继安装而现在未监听 → 强制重走 sync（新端口 → 幂等键变化 → 重装 env）；中继启动/停止留痕日志；停用卸载补日志。新增死端口自愈回归测试，合计 119 项全绿。

### v0.7.2
- **JSON 树缩进修复**：嵌套层缺一级缩进（`.npx-json-kids` 容器无边距），树形层级显示拍平；新增真实 React 加载 client.js 工厂的 SSR 冒烟测试（工厂可执行、exports 完整、图表聚合可调用），合计 118 项全绿。

### v0.7.1
- **严重修复：harness 层「先还原再装」误杀本地中继**——`sync()` 先启动中继、再还原旧策略、再装新策略，但中间还原步骤把刚启动的中继一并停掉，导致策略指向无人监听的端口：日志与自检全部正常（纯路由判断），实际 `web_fetch` 全断。v0.5.0–v0.7.0 均受影响；修复后中间还原不再触碰中继（新增 3 项生命周期回归测试，合计 116 项全绿）。

### v0.7.0
- **JSON 树渲染**：请求/响应体是合法 JSON 时自动切换为可折叠的树视图（键/字符串/数字/布尔分色，默认展开前 3 层，点击逐层展开）——纯 React 元素递归自绘，不引入 JSON 库、不做 innerHTML。
- **查看全部内容**：展开详情新增「查看全部请求/响应内容」按钮——服务端保留完整内容（单条 ≤256KB、总池 ≤4MB 超限挤出最旧，内存仍有硬上限），预览只显示前 512B 的限制不再影响查看全量；二进制与超限内容如实标注原因。
