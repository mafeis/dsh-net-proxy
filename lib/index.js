...
export function apply(ctx, config) {
  ...
  function refreshProxy() {
    ...
    if (cfg.enabled) {
      ...
      wrapped = true;
      logger.error(`[net-proxy] 已启用代理 ${proxy.protocol}://${proxy.host}:${proxy.port}`);
    } else if (wrapped) {
      globalThis.fetch = originalFetch;
      wrapped = false;
      logger.error("[net-proxy] 已停用代理（直连）");
    }
  }
  ...
  function reloadFrom(f) {
    ...
    try {
      ...
      cfg = next;
      refreshProxy();
      if (changed) logger.error(`[net-proxy] 配置已加载: ${JSON.stringify({ ...cfg, password: cfg.password ? "***" : "" })}`);
    } catch (err) {
      logger.error("[net-proxy] 热更失败:", err && err.message);
    }
  }
  ...
  ctx.inject(["webServer"], function (webCtx) {
    webCtx.effect(function () {
      ...
      logger.error("[net-proxy] 同源设置路由: /_dsh/net-proxy");
      return dispose;
    }, "net-proxy: web settings route");
  });

  logger.error(`[net-proxy] 配置文件: ${file}（enabled=${cfg.enabled}）`);
  refreshProxy();
