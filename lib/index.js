...
function settingsHandler(req, res, file, reloadFn) {
  ...
  if (req.method === "POST") {
    ...
    req.on("end", function () {
      try {
        const next = toCfg(JSON.parse(body || "{}"));
  ...
  ctx.inject(["webServer"], function (webCtx) {
    webCtx.effect(function () {
      const dispose = webCtx.webServer.register({
        kind: "exact",
        path: "/_dsh/net-proxy",
        handler: function (req, res) { settingsHandler(req, res, file, function () { reloadFrom(file); }); },
      });
