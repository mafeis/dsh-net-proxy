...
export async function proxiedFetch(input, init = {}, proxy, originalFetch) {
  ...
  if (urlObj.protocol === "https:") {
    ...
    const h = { Host: urlObj.host, ...headers };
...
async function sendViaSocket(sock, url, head, body) {
  ...
  if (te.includes("chunked")) {
    bodyBuf = await stream.readChunked();
  } else if (hdrs["content-length"] != null) {
    bodyBuf = await stream.readExactly(Number(hdrs["content-length"]));
  } else {
    bodyBuf = await stream.readToEnd();
  }
  sock.destroy(); // 读完即关，释放连接

  // 返回标准 Response：真 body 流 + Headers 实例。
  // 插件把 dsh 的 fetch 换成 proxiedFetch 后，dsh 用的是标准 Response 语义
  // （response.body.getReader() 读 SSE 流、headers.get() 等）。之前返回 plain
  // 对象（无 body、headers 非 Headers 实例）会导致 dsh 启用代理即报错：
  //   TypeError: Cannot read properties of undefined (reading 'getReader')
  //   TypeError: r.headers.get is not a function
  const m = statusLine.match(/^HTTP\/\d\.\d \d{3}(?: (.+))?$/);
  const statusText = (m && m[1] ? m[1] : "").trim();
  const response = new Response(new Uint8Array(bodyBuf), {
    status,
    statusText,
