...
function readExactly(sock, n) {
  return new Promise((resolve, reject) => {
    ...
    function onData(c) {
      ...
      got += c.length;
      if (got >= n) {
        const full = chunks.length === 1 ? chunks[0] : Buffer.concat(chunks);
        const out = full.subarray(0, n);
        const rest = full.subarray(n);
        cleanup();
        if (rest.length > 0) { try { sock.unshift(rest); } catch {} }
        resolve(out);
      }
    }
      ...
      req.on("response", (h) => {
        try {
          ...
          const streamBody = new ReadableStream({
            start(c) {
              let dec = null, closed = false;
              const close = () => { if (closed) return; closed = true; try { c.close(); } catch {} };
              if (ce === "gzip") dec = zlib.createGunzip();
              else if (ce === "deflate") dec = zlib.createInflate();
              else if (ce === "br") dec = zlib.createBrotliDecompress();
              if (dec) {
                dec.on("data", (x) => { try { c.enqueue(new Uint8Array(x)); } catch {} });
                dec.on("end", close);
                dec.on("error", (e) => { try { c.error(e); } catch {} });
              }
              req.on("data", (d) => { if (dec) { if (!dec.write(d)) req.pause(); } else { try { c.enqueue(new Uint8Array(d)); } catch {} } });
              if (dec) dec.on("drain", () => req.resume());
              req.on("end", () => { if (dec) dec.end(); else close(); try { client.close(); } catch {} });
              req.on("error", (e) => { if (dec) { try { dec.destroy(); } catch {} } try { c.error(e); } catch {} });
            },
          });
