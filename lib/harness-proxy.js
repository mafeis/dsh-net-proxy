...
export function createHarnessProxySync({ report = () => {}, loadModule = loadHarnessProxyModule, relay } = {}) {
  ...
  /**
   * @param stopRelay 是否连本地中继一起停：sync(null)/dispose 卸载时为 true；
   *                  「先还原再装」的中间还原为 false——中继是刚为本次安装启动的，
   *                  在这里停掉会让策略指向无人监听的端口（web_fetch 全断）。
   */
  async function teardown(stopRelay = true) {
  ...
  async function sync(proxy) {
    ...
    if (!proxy) {
      ...
      if (had) report("harness 代理层已卸载（代理停用）");
      return;
    }
    ...
    if (!loaded || !loaded.mod) {
      // 中继已为本轮启动但 harness 模块不可用：停掉中继防回环孤儿监听
      //（看护只认状态里的 relayPort，此分支不写 relayPort，泄漏 socket 永无人回收）。
      await teardown();
    ...
    try {
      ...
      await teardown(); // 安装失败：本轮启动的中继无人指向，一并停掉防孤儿监听
