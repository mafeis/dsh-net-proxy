...
async function proxiedOnce(url, init = {}, proxy, originalFetch) {
  ...
  if (urlObj.protocol === "https:") {
    ...
    await new Promise((resolve, reject) => {
      ...
      tlsSock.once("secureConnect", () => { clearTimeout(timer); tlsSock.removeAllListeners("error"); resolve(); });
      tlsSock.once("error", (e) => { clearTimeout(timer); reject(ProxyError("ETLS", `TLS to ${targetHost} failed: ${e.message}`, e)); });
    });
    if (tlsSock.alpnProtocol === "h2") {
      // C2：服务器协商出 HTTP/2 → 走 HTTP/2；否则回退 HTTP/1.1
      return sendViaHttp2(tlsSock, targetHost, targetPort, hostHeader, urlObj, method, headers, body, signal, { timeoutMs });
    }
    const path = urlObj.pathname + urlObj.search;
    const h = { Host: hostHeader, Connection: "close", ...headers };
    const head = `${method} ${path} HTTP/1.1\r\n${Object.entries(h).map(([k, v]) => `${k}: ${v}`).join("\r\n")}\r\n\r\n`;
    return sendViaSocket(tlsSock, url, head, body, { signal, timeoutMs });
  }
  ...
  if (isSocks) {
    ...
    return sendViaSocket(tunneled, url, socksHead, body, { signal, timeoutMs });
  }
  ...
  return sendViaSocket(sock, url, httpHead, body, { signal, timeoutMs });
}
...
export async function proxiedFetch(input, init = {}, proxy, originalFetch) {
  ...
  let cur = { ...(init || {}) };
  if (input && typeof input === "object" && !(input instanceof URL) && input.url && !cur.method) cur.method = input.method || "GET";
  for (let i = 0; i < 6; i++) {
    try {
