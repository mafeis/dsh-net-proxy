...
async function proxiedOnce(url, init = {}, proxy, originalFetch) {
  ...
  if (urlObj.protocol === "https:") {
    ...
    await new Promise((resolve, reject) => {
      ...
      tlsSock.once("secureConnect", () => { cleanup(); tlsSock.removeAllListeners("error"); resolve(); });
      tlsSock.once("error", (e) => { cleanup(); reject(ProxyError("ETLS", `TLS to ${targetHost} failed: ${e.message}`, e)); });
    });
