...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var css =
			...
			// ── 日志列表（v0.7.8 重设计）：左缘状态条 + 方法徽章 + 单行主体 + 弱化次要列 ──
			".npx-log-item{position:relative;border:1px solid var(--dsw-alias-border-l2,#242b38);border-radius:9px;padding:7px 12px 7px 14px;background:linear-gradient(180deg,rgba(255,255,255,.022),rgba(255,255,255,0)) var(--dsw-alias-bg-layer-1,#12151b);cursor:pointer;transition:border-color .14s,background .14s;overflow:hidden}",
			".npx-log-item::before{content:'';position:absolute;left:0;top:0;bottom:0;width:3px;background:var(--edge,#3a4354);transition:width .14s}",
			".npx-log-item:hover{border-color:#33405a;background:linear-gradient(180deg,rgba(79,140,255,.05),rgba(79,140,255,.015)) var(--dsw-alias-bg-layer-1,#12151b)}",
			".npx-log-item:hover::before{width:5px}",
			".npx-log-item.sel{border-color:rgba(79,140,255,.55);background:linear-gradient(180deg,rgba(79,140,255,.09),rgba(79,140,255,.03)) var(--dsw-alias-bg-layer-1,#12151b)}",
			".npx-log-item.ok{--edge:#2e9e5b}.npx-log-item.err{--edge:#d64545}.npx-log-item.live{--edge:#8b93a3}",
			".npx-log-line{display:flex;gap:9px;align-items:center;font-size:12px;flex-wrap:nowrap;min-width:0}",
			".npx-log-m{flex:none;font-size:10.5px;font-weight:700;letter-spacing:.5px;color:#b9c6da;background:rgba(148,163,184,.12);border:1px solid rgba(148,163,184,.16);border-radius:5px;padding:1px 7px;white-space:nowrap}",
			".npx-log-u{color:#e2eaf5;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1;min-width:0;font-size:12.5px}",
			".npx-log-b{color:#7d8aa0;white-space:nowrap;font-size:11px;font-variant-numeric:tabular-nums}",
			".npx-log-s{flex:none;min-width:34px;text-align:right;font-weight:700;font-size:12px;font-variant-numeric:tabular-nums}",
			".npx-log-s.ok{color:#41d97f}.npx-log-s.err{color:#ff6b6b}",
			// 一次点击的轻量展开：关键元信息一行，不用弹窗
			".npx-log-quick{display:flex;gap:14px;align-items:center;flex-wrap:wrap;padding:7px 2px 2px;margin-top:6px;border-top:1px dashed rgba(148,163,184,.18);font-size:11px;color:#93a2ba}",
			".npx-log-quick b{color:#c4d1e4;font-weight:600}",
			".npx-log-detail-btn{margin-left:auto;appearance:none;border:1px solid rgba(79,140,255,.45);background:rgba(79,140,255,.1);color:#7db8ff;border-radius:7px;padding:3px 12px;font-size:11px;cursor:pointer;transition:background .14s,border-color .14s;white-space:nowrap}",
			".npx-log-detail-btn:hover{background:rgba(79,140,255,.2);border-color:rgba(79,140,255,.7)}",
			".npx-log-e{color:var(--dsw-alias-label-danger,#e74c3c);font-size:11px;margin-top:4px;word-break:break-all}",
			// ── 日志详情弹窗（v0.7.8 精修）：深底卡片 + 顶缘光带 + 徽章行 + 分区头部 ──
			".npx-modal-mask{position:fixed;inset:0;background:rgba(5,7,11,.66);backdrop-filter:blur(4px);z-index:9999;display:flex;align-items:center;justify-content:center;padding:24px;animation:npxFadeIn .16s ease}",
			...
			".npx-modal{position:relative;width:min(880px,96vw);max-height:88vh;display:flex;flex-direction:column;border:1px solid #2e3850;border-radius:16px;background:linear-gradient(180deg,#161b26 0%,#111520 100%);box-shadow:0 24px 80px rgba(0,0,0,.6),0 0 0 1px rgba(79,140,255,.1),0 1px 0 rgba(255,255,255,.06) inset;animation:npxSlideUp .2s cubic-bezier(.22,1,.36,1);overflow:hidden}",
			".npx-modal::before{content:'';position:absolute;inset:0 0 auto 0;height:2px;background:linear-gradient(90deg,transparent 5%,rgba(79,140,255,.75) 30%,rgba(77,208,225,.6) 60%,transparent 95%);pointer-events:none}",
			".npx-modal-head{display:flex;align-items:center;gap:12px;padding:16px 20px 13px;border-bottom:1px solid rgba(148,163,184,.14);flex:none;background:rgba(255,255,255,.015)}",
			".npx-modal-title{font-size:13px;font-weight:600;color:#e8eef7;flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;font-family:ui-monospace,'Cascadia Code',Consolas,monospace}",
			".npx-modal-close{flex:none;appearance:none;border:1px solid rgba(148,163,184,.25);background:transparent;color:#93a2ba;border-radius:8px;width:28px;height:28px;cursor:pointer;font-size:13px;line-height:1;transition:all .14s}",
			".npx-modal-close:hover{border-color:rgba(255,107,107,.6);color:#ff6b6b;background:rgba(255,107,107,.08)}",
			".npx-modal-meta{display:flex;gap:7px;flex-wrap:wrap;padding:12px 20px 0;flex:none}",
			".npx-modal-meta .npx-chip{font-size:11px;padding:3px 10px;border-radius:99px;border:1px solid rgba(148,163,184,.2);color:#aab6c9;background:rgba(148,163,184,.06);white-space:nowrap;font-variant-numeric:tabular-nums}",
			".npx-modal-meta .npx-chip.ok{color:#41d97f;border-color:rgba(65,217,127,.4);background:rgba(65,217,127,.08);font-weight:600}",
			".npx-modal-meta .npx-chip.err{color:#ff6b6b;border-color:rgba(255,107,107,.4);background:rgba(255,107,107,.08);font-weight:600}",
			".npx-modal-body{flex:1;overflow-y:auto;padding:14px 20px 20px;display:flex;flex-direction:column;gap:16px;min-height:0}",
			".npx-modal-body::-webkit-scrollbar{width:8px}.npx-modal-body::-webkit-scrollbar-thumb{background:rgba(148,163,184,.22);border-radius:4px}",
			...
			".npx-modal-sec-h::after{content:'';flex:1;height:1px;background:linear-gradient(90deg,rgba(148,163,184,.22),transparent)}",
			".npx-modal-sec-h.req{color:#6cc6ff}.npx-modal-sec-h.req::before{background:#6cc6ff;box-shadow:0 0 6px rgba(108,198,255,.6)}",
			".npx-modal-sec-h.res{color:#8ddb8c}.npx-modal-sec-h.res::before{background:#8ddb8c;box-shadow:0 0 6px rgba(141,219,140,.6)}",
			".npx-json-pre{margin:8px 0 0;padding:8px;border-radius:6px;background:#12161f;border:1px solid #262e3d;font-family:ui-monospace,'Cascadia Code',Consolas,monospace;font-size:12px;line-height:1.6;color:#dbe2ee;white-space:pre-wrap;word-break:break-all}",
			...
			".npx-json{font-family:ui-monospace,'Cascadia Code',Consolas,'JetBrains Mono',monospace;font-size:12px;line-height:1.7;color:#dbe2ee;padding:10px 12px;border-radius:8px;background:#12161f;border:1px solid #262e3d;user-select:text}",
			".npx-json .jkey{color:#59c2ff;font-weight:600}",
			".npx-json .jstr{color:#8ddb8c}",
			...
			".npx-json .jnum{color:#ffb454}",
			".npx-json .jbool,.npx-json .jnull{color:#e6b3ff;font-weight:600}",
			".npx-json .jdim{color:#6b7690}",
			".npx-json .jbracket{color:#8fa0b8}",
		...
		function JsonScalar(props) {
			...
			if (ty === "string") {
				...
				if (s.length > 120) {
					var [clip, setClip] = React.useState(false);
					return clip
		...
		function LogItem(props) {
			...
			return react_jsx_runtime.jsxs("div", { className: "npx-log-item " + stCls + (open ? " sel" : ""), onClick: function () { props.onToggle(it.id, it); }, children: [
				...
				open ? react_jsx_runtime.jsxs("div", { className: "npx-log-quick", onClick: function (e) { e.stopPropagation(); }, children: [
					...
					it.error ? react_jsx_runtime.jsx("span", { style: { color: "#ff6b6b" }, children: String(it.error).slice(0, 90) }) : null,
