...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var en = {
			...
			followInactive: "Not effective (following system): ",
		...
		function NetProxySection(props) {
			...
			// 跟随横幅文字：不是写死的「当前生效」——按实际转发状态取词，
			// 颜色（绿/红）与文字保持一致，杜绝「绿字写着生效实际没转」类的矛盾。
			var followBannerText = (on ? t("followEffective") : t("followInactive")) + " " + (sysInfo ? sysInfo.proxy : "");
			...
			return react_jsx_runtime.jsx("div", { className: "npx-root", children: [
				...
				tab === "log" ? react_jsx_runtime.jsx(LogPanel, { t: t }) : tab === "charts" ? react_jsx_runtime.jsx(ChartPanel, { t: t }) : react_jsx_runtime.jsx("section", { className: "npx-panel", children: [
					...
					// 红/绿横幅 + 动态文字：绿=「当前生效（跟随系统）」红=「未生效（跟随系统）」，
					// 文字随实际转发状态取词，与颜色永远一致。
