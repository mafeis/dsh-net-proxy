...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var css =
			...
			".npx-probe-stat span{font-size:11px;color:var(--dsw-alias-label-tertiary)}"].join("");
		...
		var zh = {
			...
			harnessInstalled: "Harness 代理层已装：web_fetch 同样走代理",
			harnessUnverified: "Harness 代理层已装（路由自检未通过，web_fetch 可能仍直连）",
			harnessSocks: "SOCKS5 暂不能装进 Harness 代理层：web_fetch 不走代理（代理若同时提供 HTTP 端口，改用 HTTP 即可覆盖）",
			harnessUnavailable: "Harness 代理层不可用：无法加载 dsh-http-proxy，web_fetch 不走代理",
			harnessError: "Harness 代理层安装失败",
			protocol: "协议",
			host: "代理地址",
			port: "端口",
			username: "用户名（可选）",
		...
		var en = {
			...
			harnessInstalled: "Harness proxy layer installed: web_fetch is proxied too",
			harnessUnverified: "Harness proxy layer installed (route self-check failed, web_fetch may still connect directly)",
			harnessSocks: "SOCKS5 can't be installed into the harness proxy layer: web_fetch stays direct (use the proxy's HTTP port to cover web_fetch)",
			harnessUnavailable: "Harness proxy layer unavailable: dsh-http-proxy not loadable, web_fetch stays direct",
			harnessError: "Harness proxy layer failed to install",
			protocol: "Protocol",
			host: "Proxy address",
			port: "Port",
		...
		function NetProxySection(props) {
			...
			// harness 代理层（web_fetch 真实走向）状态一行；mode=off（未启用）不展示
			var harnessLine = null;
			if (harnessInfo && harnessInfo.mode && harnessInfo.mode !== "off") {
				if (harnessInfo.mode === "installed") harnessLine = harnessInfo.verified === false ? [t("harnessUnverified"), "warn"] : [t("harnessInstalled"), "ok"];
				else if (harnessInfo.mode === "unsupported-socks") harnessLine = [t("harnessSocks"), "warn"];
				else if (harnessInfo.mode === "unavailable") harnessLine = [t("harnessUnavailable"), "warn"];
				else if (harnessInfo.mode === "error") harnessLine = [t("harnessError") + (harnessInfo.error ? "：" + harnessInfo.error : ""), "warn"];
			}
			...
			return react_jsx_runtime.jsx("div", { className: "npx-root", children: [
				...
				react_jsx_runtime.jsx("section", { className: "npx-panel", children: [
					...
					harnessLine ? react_jsx_runtime.jsx("div", { className: "npx-field-hint", style: { color: harnessLine[1] === "ok" ? "var(--dsw-alias-label-success,#2ecc71)" : "var(--dsw-alias-label-warning,#f1c40f)" }, children: harnessLine[0] }) : null,
