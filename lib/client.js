...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var css =
			...
			".npx-log-m{font-weight:600;color:var(--dsw-alias-label-secondary);min-width:44px;white-space:nowrap}",
			".npx-log-u{color:var(--dsw-alias-label-primary);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1;min-width:0}",
			".npx-log-b{color:var(--dsw-alias-label-tertiary);white-space:nowrap;font-size:11px}",
			".npx-log-s.ok{color:#57e389;font-weight:600}",
			".npx-log-s.err{color:#ff7b72;font-weight:600}",
			...
			".npx-json-pre{margin:8px 0 0;padding:8px;border-radius:6px;background:#171c26;border:1px solid #232a38;font-family:var(--dsw-font-mono,monospace);font-size:11px;line-height:1.5;color:#c9d2e0;white-space:pre-wrap;word-break:break-all}",
			...
			".npx-json{font-family:var(--dsw-font-mono,monospace);font-size:11.5px;line-height:1.65;color:#c9d2e0;padding:10px 12px;border-radius:8px;background:#171c26;border:1px solid #232a38;user-select:text}",
			".npx-json .jkey{color:#6cc6ff;font-weight:600}",
			".npx-json .jstr{color:#9ee493}",
			...
			".npx-json .jnum{color:#ffcb6b}",
			".npx-json .jbool,.npx-json .jnull{color:#d6a2ff;font-weight:600}",
			".npx-json .jdim{color:#7a8496}",
			".npx-json .jbracket{color:#94a3b8}",
		...
		function LogItem(props) {
			...
			return react_jsx_runtime.jsx("div", { className: "npx-log-item", onClick: function () { props.onOpen(it.id); }, children: [
			...
			return react_jsx_runtime.jsx("section", { className: "npx-panel", children: [
				...
				entries.length ? react_jsx_runtime.jsxs("div", { children: [
					...
					shown.length ? react_jsx_runtime.jsx("div", { className: "npx-log-list", children: shown.map(function (it) {
						return react_jsx_runtime.jsx(LogItem, { t: t, it: it, onOpen: function (id) { setExpanded(id); } }, it.id);
				...
				// 日志详情弹窗（v0.7.6）：按 id 从最新数据里取条目（3s 轮询自动刷新弹窗内容）
				expanded != null && entries.some(function (x) { return x.id === expanded; }) ? (function () {
					var cur = entries.filter(function (x) { return x.id === expanded; })[0];
					return react_jsx_runtime.jsx(LogDetailModal, { t: t, it: cur, onClose: function () { setExpanded(null); } });
				})() : null,
			] });
		}
