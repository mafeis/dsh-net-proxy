...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var css =
			...
			".npx-chart-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}",
			".npx-chart-card{position:relative;border:1px solid var(--dsw-alias-border-l2,#2a2f3a);border-radius:12px;padding:14px 14px 12px;background:linear-gradient(180deg,var(--dsw-alias-bg-layer-1,#141821) 0%,var(--dsw-alias-bg-layer-2,#1a1e26) 100%);min-width:0;overflow:hidden;transition:border-color .18s,box-shadow .18s}",
			...
			".npx-stat-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}",
			".npx-stat{position:relative;display:flex;flex-direction:column;gap:3px;padding:12px 14px 11px;border:1px solid var(--dsw-alias-border-l2,#2a2f3a);border-radius:12px;background:linear-gradient(180deg,var(--dsw-alias-bg-layer-1,#141821) 0%,var(--dsw-alias-bg-layer-2,#1a1e26) 100%);overflow:hidden}",
			...
			".npx-stat-value{font-size:20px;font-weight:650;color:var(--dsw-alias-label-primary);font-variant-numeric:tabular-nums;line-height:1.15}",
			".npx-stat-value.err{color:var(--dsw-alias-label-danger,#e74c3c)}",
			".npx-stat-value.ok{color:var(--dsw-alias-label-success,#2ecc71)}",
			".npx-hbar-row{display:grid;grid-template-columns:minmax(90px,160px) 1fr auto;gap:10px;align-items:center;font-size:11.5px;padding:4px 6px;border-radius:7px;transition:background .15s}",
		...
		function TimelineChart(props) {
			var d = props.data;
			var W = 560, H = 150, padL = 10, padR = 10, padT = 14, padB = 18;
