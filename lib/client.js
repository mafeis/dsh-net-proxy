...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var zh = {
			...
			enable: "启用代理",
			follow: "跟随系统代理",
			followHint: "开启后每 3 秒检测操作系统的系统代理：系统开启则自动启用（v2rayN/Clash 切换端口会自动跟随），系统关闭则自动直连。手动修改下方地址/端口/协议并保存时，会自动关闭跟随。",
			sysOk: "系统代理已启用",
			sysOff: "系统代理未开启（已自动直连）",
			sysPac: "系统为 PAC 模式，暂不支持跟随（沿用手动配置）",
			sysUnavailable: "无法读取系统代理设置（沿用手动配置）",
			followManualNote: "跟随中：实际使用上方检测到的系统代理；下方地址/端口仅作为系统读取失败时的兜底，不会自动改变。",
		...
		var en = {
			...
			followHint: "Checks the OS proxy settings every 3s while on: follows the system proxy (v2rayN/Clash port changes included) and falls back to direct when it is off. Saving manual host/port/protocol changes turns following off automatically.",
			...
			followManualNote: "While following, the detected system proxy above is what actually applies; the address/port below are only a fallback and will not change.",
		...
		function NetProxySection(props) {
			...
			function refreshStatus() {
				fetch(API).then(function (r) { return r.json(); }).then(function (j) {
					if (j && j.ok) { if (j.system) setSysInfo(j.system); setReady(true); setUnreachable(false); }
				}).catch(function () {});
			}
			...
			function reloadNow() {
				...
				fetch(API).then(function (r) { return r.json(); }).then(function (j) {
					if (j && j.ok) { applyValue(j.value || {}); if (j.system) setSysInfo(j.system); setReady(true); setUnreachable(false); }
				}).catch(function () { setUnreachable(true); });
			}
			...
			if (followSystem && sysInfo) {
				if (sysInfo.state === "ok") followStateText = t("sysOk") + "：" + sysInfo.proxy + "（" + t("followOn") + "）";
			...
			return react_jsx_runtime.jsx("div", { className: "npx-root", children: [
				...
				react_jsx_runtime.jsx("section", { className: "npx-panel", children: [
					...
					followSystem ? react_jsx_runtime.jsx("div", { className: "npx-field-hint", children: [
						react_jsx_runtime.jsx("div", { children: t("followHint") }),
					...
					following ? react_jsx_runtime.jsx("div", { className: "npx-alert ok", children: [
						react_jsx_runtime.jsx("div", { style: { fontWeight: 600 }, children: t("followEffective") }),
						react_jsx_runtime.jsx("div", { style: { marginTop: 2 }, children: sysInfo.proxy }),
					] }) : null,
