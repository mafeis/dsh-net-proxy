...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var css =
			...
			".npx-log-item{border:1px solid var(--dsw-alias-border-l2,#2a2f3a);border-radius:8px;padding:8px 10px;background:var(--dsw-alias-bg-layer-1,#12151b);cursor:pointer;transition:border-color .15s,background .15s}",
			".npx-log-item:hover{border-color:var(--dsw-alias-brand-primary,#4f8cff);background:rgba(79,140,255,.06)}",
			".npx-log-line{display:flex;gap:8px;align-items:baseline;font-size:12px;flex-wrap:nowrap;min-width:0}",
			".npx-log-m{font-weight:600;color:#9fb0c8;min-width:44px;white-space:nowrap}",
			".npx-log-u{color:#dfe7f2;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:1;min-width:0}",
			".npx-log-b{color:#8fa0b8;white-space:nowrap;font-size:11px}",
			".npx-log-s.ok{color:#4ade80;font-weight:700}",
			".npx-log-s.err{color:#f87171;font-weight:700}",
			".npx-log-e{color:var(--dsw-alias-label-danger,#e74c3c);font-size:11px;margin-top:4px;word-break:break-all}",
			// ── 日志详情弹窗（v0.7.6）：居中卡片 + 遮罩；详情区内部滚动（弹窗场景不与页面滚动叠加）──
			...
			"@keyframes npxSlideUp{from{transform:translateY(10px);opacity:.6}to{transform:translateY(0);opacity:1}}",
			".npx-modal{width:min(860px,96vw);max-height:86vh;display:flex;flex-direction:column;border:1px solid var(--dsw-alias-border-l1,#39414f);border-radius:14px;background:linear-gradient(180deg,var(--dsw-alias-bg-layer-1,#141821),var(--dsw-alias-bg-layer-0,#10141c));box-shadow:0 18px 60px rgba(0,0,0,.5),0 0 0 1px rgba(79,140,255,.08);animation:npxSlideUp .18s ease}",
			".npx-modal-head{display:flex;align-items:center;gap:10px;padding:14px 18px 12px;border-bottom:1px solid var(--dsw-alias-border-l2,#2a2f3a);flex:none}",
			".npx-modal-title{font-size:13px;font-weight:600;color:var(--dsw-alias-label-primary);flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}",
			".npx-modal-close{flex:none;appearance:none;border:1px solid var(--dsw-alias-border-l2,#2a2f3a);background:transparent;color:var(--dsw-alias-label-secondary);border-radius:8px;width:28px;height:28px;cursor:pointer;font-size:14px;line-height:1}",
			".npx-modal-close:hover{border-color:var(--dsw-alias-brand-primary,#4f8cff);color:var(--dsw-alias-label-primary)}",
			".npx-modal-meta{display:flex;gap:6px;flex-wrap:wrap;padding:10px 18px 0;flex:none}",
			".npx-modal-meta .npx-chip{font-size:11px;padding:2px 9px;border-radius:99px;border:1px solid var(--dsw-alias-border-l2,#2a2f3a);color:var(--dsw-alias-label-secondary);white-space:nowrap}",
			".npx-modal-meta .npx-chip.ok{color:#57e389;border-color:rgba(87,227,137,.4)}",
			".npx-modal-meta .npx-chip.err{color:#ff7b72;border-color:rgba(255,123,114,.4)}",
			".npx-modal-body{flex:1;overflow-y:auto;padding:12px 18px 18px;display:flex;flex-direction:column;gap:14px;min-height:0}",
			".npx-modal-sec-h{display:flex;align-items:center;gap:8px;font-size:11.5px;font-weight:600;letter-spacing:.4px;color:var(--dsw-alias-label-secondary);margin:2px 0 6px}",
			".npx-modal-sec-h::after{content:'';flex:1;height:1px;background:linear-gradient(90deg,var(--dsw-alias-border-l2,#2a2f3a),transparent)}",
			".npx-modal-sec-h.req{color:#7ec8ff}.npx-modal-sec-h.res{color:#a8e08f}",
		...
		function LogItem(props) {
			...
			return react_jsx_runtime.jsx("div", { className: "npx-log-item", onClick: function () { props.onOpen(it.id, it); }, children: [
		...
		function LogPanel(props) {
			...
			var expandedRef = React.useRef(null); // 弹窗条目快照：被轮询淘汰时兜底显示
			// 列表不内滚（避免与宿主页面滚动叠加成双滚动条）：先取 80 条，按需加载全部
			var [limit, setLimit] = React.useState(80);
			var [filter, setFilter] = React.useState("");
			var openDetail = function (id, it) { expandedRef.current = it || null; setExpanded(id); };
			var closeDetail = function () { expandedRef.current = null; setExpanded(null); };
			...
			// 弹窗条目：优先用最新数据（内容随轮询刷新），被淘汰时回落到快照
			var cur = expanded != null ? (entries.filter(function (x) { return x.id === expanded; })[0] || expandedRef.current) : null;
			return react_jsx_runtime.jsx("section", { className: "npx-panel", children: [
				...
				entries.length ? react_jsx_runtime.jsxs("div", { children: [
					...
					shown.length ? react_jsx_runtime.jsx("div", { className: "npx-log-list", children: shown.map(function (it) {
						return react_jsx_runtime.jsx(LogItem, { t: t, it: it, onOpen: openDetail }, it.id);
				...
				// 日志详情弹窗（v0.7.6）：按 id 从最新数据取条目（3s 轮询自动刷新弹窗内容）；
				// 被轮询淘汰时回落到打开时的快照，弹窗不会闪没，只有用户主动关闭才消失
				expanded != null && cur ? react_jsx_runtime.jsx(LogDetailModal, { t: t, it: cur, onClose: closeDetail }) : null,
			] });
		}
