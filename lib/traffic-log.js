...
/** 字节块 → 预览字符串：前 previewBytes 字节按 UTF-8 解码，控制字符以 · 展示。 */
 ...
 */
export function createTrafficLog({ maxEntries = 500, previewBytes = 512 } = {}) {
  let entries = []; // 新的在前
  let nextId = 1;
  let cfg = { previewBytes };
  const sum = { total: 0, upBytes: 0, downBytes: 0, errors: 0, since: Date.now() };
   ...
   * handle 全部方法对 null 安全由调用侧 `entry?.` 保证——begin 在禁用时仍返回 handle，
   * 但内部为 no-op（这样协议栈不需要判 log 开关）。
   */
  function begin(meta) {
    const m = meta || {};
    const entry = {
  ...
  function summary() { return { ...sum }; }
  function list(limit = 200) {
    const n = Math.max(1, Math.min(Number(limit) || 200, maxEntries));
    return entries.slice(0, n);
  }
  function liveCount() { return entries.filter((e) => e.live).length; }
  function clear() { entries = []; sum.total = 0; sum.upBytes = 0; sum.downBytes = 0; sum.errors = 0; sum.since = Date.now(); }
